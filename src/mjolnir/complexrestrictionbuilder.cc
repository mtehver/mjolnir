...
void ComplexRestrictionBuilder::set_from_id(const uint64_t from_id) {
  from_id_ = from_id;
}

// Set the to edge id.
void ComplexRestrictionBuilder::set_to_id(const uint64_t to_id) {
  to_id_ = to_id;
}
...
void ComplexRestrictionBuilder::set_via_list(
    const std::vector<uint64_t>& via_list) {
...
std::size_t ComplexRestrictionBuilder::BaseSizeOf() const {
  ...
  size += (via_list_.size() * sizeof(uint64_t));
  return size;
}
...
std::ostream& operator<<(std::ostream& os, const ComplexRestrictionBuilder& crb) {
  ...
  // Write out the bytes
  os.write(reinterpret_cast<const char*>(&crb.from_id_), sizeof(uint64_t));
  os.write(reinterpret_cast<const char*>(&crb.to_id_), sizeof(uint64_t));
